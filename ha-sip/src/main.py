...
def load_menu_from_file(file_name: Optional[str], sip_account_index: int) -> Optional[incoming_call.IncomingCallConfig]:
    if not file_name:
        logging.warning('No file name for incoming call config specified, account: %s', sip_account_index)
        return None
    try:
        with open(file_name) as stream:
            content = yaml.safe_load(stream)
            logging.info('Loaded menu for incoming call, account: %s', sip_account_index)
            return content
    except BaseException as e:
        logging.error('Error loading menu for incoming call: %s, account: %s', e, sip_account_index)
        return None
...
def get_name_server(raw_name_server: str):
    ...
    if name_server_without_empty:
        logging.info('Setting name server: %s', name_server)
...
def get_cache_dir(raw_cache_dir: str) -> Optional[str]:
    ...
    if not os.path.isdir(raw_cache_dir):
        ...
        return None
    logging.info("Found cache directory '%s'", raw_cache_dir)
...
def main():
    ...
    for key, account_config in account_configs.items():
        if account_config.enabled:
            sip_accounts[key] = account.create_account(end_point, account_config, command_handler, ha_config, mqtt_client, is_first_enabled_account)
...
class CustomFormatter(logging.Formatter):
    ...
    bold_red = "\x1b[31;1m"
    reset = "\x1b[0m"
    format_part1 = "%(asctime)s - %(levelname)-8s - %(filename)20s:%(lineno)3d - "
    format_part2 = "[%(threadName)s %(name)s.%(funcName)s]: %(message)s"

    FORMATS = {
        logging.DEBUG: format_part1 + grey + format_part2 + reset,
        logging.INFO: format_part1 + green + format_part2 + reset,
        logging.WARNING: format_part1 + yellow + format_part2 + reset,
        logging.ERROR: format_part1 + red + format_part2 + reset,
        logging.CRITICAL: format_part1 + bold_red + format_part2 + reset
    }
...
if __name__ == '__main__':
    logging.basicConfig(level=logging.DEBUG, format='[%(asctime)s] %(levelname)-8s - {%(pathname)s:%(lineno)d} - %(name)s.%(funcName)s: %(message)s')
